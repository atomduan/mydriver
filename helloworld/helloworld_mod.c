...
/*
 * cat /proc/devices (the major_num is not precise here) 
 * or ll /dev (the major_num is precise here) to check whether the MAJOR_NUM has been ocupied.
 * for example: the 250 MAJOR_NUM are ocupied by watchdog device(module?).
 *
 * check module: lsmod, ll /dev, cat /proc/devices
 */
...
{
    int err = 0;
    printk("+helloworld_mod_ioctl()!\n");
    printk("    +switch()\n");
    switch(cmd)
    {
        case HELLOWORLD_IOCTL_RESETLANG:
            printk("        >in case: HELLOWORLD_IOCTL_RESETLANG\n");
            langtype = english;
            break;
        case HELLOWORLD_IOCTL_GETLANG:
            printk("        >in case: HELLOWORLD_IOCTL_GETLANG\n");
            err = copy_to_user((int *)arg, &langtype, sizeof(int));
            break;
        case HELLOWORLD_IOCTL_SETLANG:
            printk("        >in case: HELLOWORLD_IOCTL_SETLANG\n");
...
{
    int result;
    printk("+helloworld_mod_init()!\n");
    devnum = MKDEV(MAJOR_NUM, MINOR_NUM);
    result = register_chrdev_region(devnum, 1, modname);

    if(result < 0)
    {
        printk("helloworld_mod : can't get major number! devnum is %u, result is %d,\n", devnum, result);
        return result;
    }    
